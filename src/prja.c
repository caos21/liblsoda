...
int prja(struct lsoda_context_t *ctx, double *y) {
  ...
  /*
     ...
     J, scaled by -_C(h) * _C(el)[1], is stored in _C(wm).  Then the norm of J (
     the matrix norm consistent with the weihted max-norm on vectors given by
     vmnorm ) is computed, and J is overwritten by P.  P is then subjected to LU
     decomposition in preparation for later solution of linear systems with p as
     coefficient matrix.  This is done by defa if _C(miter) = 2, and by dgbfa if
     _C(miter) = 5.
  */
  _C(nje)++;
  hl0 = _C(h) * _C(el)[1];
