...
#define resetcoeff()                                                     \
  {                                                                      \
    int i;                                                               \
                                                                         \
    double el0 = _C(el)[1];                                              \
    for (i = 1; i <= (_C(nq) + 1); i++) _C(el)[i] = _C(elco)[_C(nq)][i]; \
...
int stoda(struct lsoda_context_t *ctx, double *y, int jstart) {
  ...
  */
  ...
  /*
     ...
     If _C(h) or _C(meth) is being changed, _C(ialth) is reset to (_C(nq) + 1) =
     _C(nq) + 1 to prevent further changes in _C(h) for that many steps.
  */
  if (jstart == -1) {
    _C(ipup) = _C(miter);
    if (_C(ialth) == 1) _C(ialth) = 2;
    if (_C(meth) != _C(mused)) {
      cfode(ctx, _C(meth));
      _C(ialth) = (_C(nq) + 1);
      resetcoeff();
  ...
  /*
     ...
     _C(rc) is the ratio of new to old values of the coefficient _C(h) *
     _C(el)[1]. When _C(rc) differs from 1 by more than ccmax, _C(ipup) is set
     to _C(miter) to force pjac to be called, if a jacobian is involved. In any
     case, prja is called at least every msbp steps.
  */
  dsm = 0.0;
  while (1) {
    /*
    ...
    */
    _C(jcur) = 0;
    while (1) {
      if (fabs(_C(rc) - 1.) > CCMAX) _C(ipup) = _C(miter);
      if (_C(nst) >= _C(nslp) + MSBP) _C(ipup) = _C(miter);
      _C(tn) += _C(h);
      for (j = _C(nq); j >= 1; j--)
        for (i1 = j; i1 <= _C(nq); i1++) {
          for (i = 1; i <= neq; i++) _C(yh)[i1][i] += _C(yh)[i1 + 1][i];
        }
      ...
      */
    if (m == 0) dsm = del / _C(tesco)[_C(nq)][2];
    if (m > 0) dsm = vmnorm(neq, _C(acor), _C(ewt)) / _C(tesco)[_C(nq)][2];
    if (dsm <= 1.) {
      /*
         ...
         consider changing _C(h) if _C(ialth) = 1.  Otherwise decrease _C(ialth)
         by 1. If _C(ialth) is then 1 and _C(nq) < maxord, then _C(acor) is
         saved for use in a possible order increase on the next step. If a
         change in _C(h) is considered, an increase or decrease in order by one
         is considered also.  A change in _C(h) is made only if it is by a
         factor of at least 1.1.  If not, _C(ialth) is set to 3 to prevent
      ...
      */
      ...
      for (j = 1; j <= (_C(nq) + 1); j++) {
        r = _C(el)[j];
        for (i = 1; i <= neq; i++) _C(yh)[j][i] += r * _C(acor)[i];
      }
      _C(icount)--;
      if (_C(icount) < 0) {
      ...
      */
      ...
      for (i = 1; i <= neq; i++) _C(yh)[maxord + 1][i] = _C(acor)[i];
      endstoda();
      break;
    }
    /* end if ( dsm <= 1. )   */
     ...
     */
       ...
       */
        ...
        if (kflag == -10) {
          ...
          _C(h) *= rh;
          for (i = 1; i <= neq; i++) y[i] = _C(yh)[1][i];
          (*ctx->function)(_C(tn), y + 1, _C(savf) + 1, ctx->data);
          _C(nfe)++;
          for (i = 1; i <= neq; i++) _C(yh)[2][i] = _C(h) * _C(savf)[i];
          _C(ipup) = _C(miter);
          _C(ialth) = 5;
          if (_C(nq) == 1) continue;
          _C(nq) = 1;
