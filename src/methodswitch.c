...
/*
 * cm1 and cm2 are calculated from tesco/elco 1 and 2 formula in
 * cfode.c:printcm12.
 *
 * */
static double cm1[13] = {
    ...
    0x1p+2,
    0x1.9435e50d79434p+0,
    0x1.c71c71c71c721p-2,
    0x1.8eaf0473189ecp-4,
    0x1.1df9ab7934513p-6,
    0x1.5b6f81b154515p-9,
    0x1.6e1dd3d149b81p-12,
    0x1.54a9415f71629p-15,
    0x1.1bcb8f930a98p-18,
...
static double cm2[13] = {
    ...
    0x1.aaaaaaaaaaaacp-3,
    0x1.9999999999999p-5,
    0x1.8eaf0473189ecp-4,
    0x1.1df9ab7934513p-6,
    0x1.5b6f81b154515p-9,
    0x1.6e1dd3d149b81p-12,
...
/* rh is an output. */
void methodswitch(struct lsoda_context_t *ctx, double dsm, double pnorm,
                  double *rh) {
  ...
  */
  if (_C(meth) == 1) {
    ...
    if (dsm <= (100. * pnorm * ETA) || _C(pdest) == 0.) {
      ...
      if ((pdh * rh1) > 0.00001) rh1it = sm1[_C(nq)] / pdh;
      rh1 = min0(rh1, rh1it);
      if (_C(nq) > mxords) {
        nqm2 = mxords;
        lm2 = mxords + 1;
