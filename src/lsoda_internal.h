...
int correction(struct lsoda_context_t *ctx, double *y, double pnorm,
               double *del, double *delp, double told, int *m);
...
#endif
#define ERROR(fmt, ...)                                                        \
  (ctx->error ? free(ctx->error) : (void)1,                                    \
   ctx->error = _strdup_printf("EE:" fmt " @(%s:%d)", ##__VA_ARGS__, __FILE__, \
                               __LINE__))
