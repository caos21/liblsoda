...
void scaleh(struct lsoda_context_t* ctx, double rh) {
  ...
  /*
     If _C(h) is being changed, the _C(h) ratio rh is checked against _C(rmax),
     hmin, and hmxi, and the _C(yh) array is rescaled.  _C(ialth) is set to
     (_C(nq) + 1) = _C(nq) + 1 to prevent a change of _C(h) for that many steps,
     unless forced by a convergence or error test failure.
  */
  rh = fmin(rh, _C(rmax));
