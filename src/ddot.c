...
double ddot(n, dx, incx, dy, incy) double *dx, *dy;
int n, incx, incy;
...

{
  ...
  if (incx == 1) {
    /*
                    ...
                    for (i = m + 1; i <= n; i = i + 5)
                            dotprod = dotprod + dx[i] * dy[i] + dx[i + 1] * dy[i
       + 1] + dx[i + 2] * dy[i + 2] + dx[i + 3] * dy[i + 3] + dx[i + 4] * dy[i +
       4];
    */
    for (i = 1; i <= n; i++) dotprod = dotprod + dx[i] * dy[i];
    return dotprod;
  }
