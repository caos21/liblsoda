...
/*
   ...
   The quantities _C(nq) = nqcur, l = _C(nq)+1, neq = neq, _C(tn), and _C(h) are
   declared static globally.  The above sum is done in reverse order. *iflag is
   returned negative if either k or t is out of bounds.
*/

{
  ...
  s = (t - _C(tn)) / _C(h);
  ic = 1;
  for (jj = (_C(nq) + 1) - k; jj <= _C(nq); jj++) ic *= jj;
  c = (double)ic;
  for (i = 1; i <= neq; i++) dky[i] = c * _C(yh)[_C(nq) + 1][i];
  for (j = _C(nq) - 1; j >= k; j--) {
    jp1 = j + 1;
    ic = 1;
    for (jj = jp1 - k; jj <= j; jj++) ic *= jj;
    c = (double)ic;
    for (i = 1; i <= neq; i++) dky[i] = c * _C(yh)[jp1][i] + s * dky[i];
  }
  if (k == 0) return 0;
  r = pow(_C(h), (double)(-k));
  for (i = 1; i <= neq; i++) dky[i] *= r;
  return 0;
} /* end intdy   */
